...
function buildGoalPayload(goal) {
  ...
  const rawProgressPercent = targetAmount === 0 ? 0 : (currentAmount / targetAmount) * 100;
  const progressPercent = Number(rawProgressPercent.toFixed(2));
  const progressBarPercent = Number(Math.min(Math.max(rawProgressPercent, 0), 100).toFixed(2));
