...
import { buildAnalyticsDateRange, buildPreviousDateRange } from "@/lib/analytics";
...
async function buildBudgetExceededAlerts({ userId, referenceDate }) {
  ...
  return budgetData.budgets
    ...
    .map((budget) => ({
      ...
      message:
        `You spent ${formatMoney(budget.spent)} against a limit of ${formatMoney(budget.amount)} in ` +
...
async function buildSavingPaceAlerts({ userId, now = new Date() }) {
  ...
  return goals
    .map((goal) => {
      ...
      const message = recommendation.isOverdue
        ? `This goal is overdue. To catch up, set aside ${formatMoney(recommendation.weeklyAmount)} per week or ${formatMoney(recommendation.monthlyAmount)} per month.`
        : `To stay on track, set aside ${formatMoney(recommendation.weeklyAmount)} per week or ${formatMoney(recommendation.monthlyAmount)} per month.`;
...
export async function getRecommendations({
  ...
  const [budgetAlerts, spendingSpikeAlerts, goalAlerts] = await Promise.all([
    buildBudgetExceededAlerts({ userId, referenceDate }),
    buildSpendingSpikeAlerts({ userId, period, from, to }),
    buildSavingPaceAlerts({ userId, now }),
  ]);
