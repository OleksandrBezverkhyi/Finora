...
async function buildBudgetExceededAlerts({ userId, referenceDate, currency = "UAH" }) {
  ...
  return budgetData.budgets
    ...
    .map((budget) => {
      const categoryLabel = budget.isOverallCategory ? "All spending" : budget.category.name;

      return {
      id: `budget-exceeded:${budget.id}`,
      type: "budget_exceeded",
      severity: "high",
      title: `${categoryLabel} limit exceeded`,
      message:
        `You have spent ${formatMoney(budget.spent, currency)} out of a limit of ${formatMoney(budget.amount, currency)} in ` +
  ...
  return [
    {
      ...
      title: "Spending increased noticeably",
      message:
        `Your spending is up by ${percentChange}% compared with the previous period, which is above the ECB's ` +
        `${ECB_SPENDING_SPIKE_BENCHMARK_PERCENT}% reference benchmark.`,
  ...
  return goals
    .map((goal) => {
      ...
      const message = recommendation.isOverdue
        ? `This goal is behind schedule. To catch up, set aside ${formatMoney(recommendation.weeklyAmount, currency)} per week or ${formatMoney(recommendation.monthlyAmount, currency)} per month.`
