...
/**
 * Converts aggregated Prisma numeric values into plain numbers for calculations.
 *
...
/**
 * Serializes monetary values into a normalized two-decimal string representation.
 *
 * @param {string | number | null | undefined} value
 * @returns {string}
 */
...
/**
 * Normalizes a transaction payload for dashboard responses.
 *
 * @param {{
...
/**
 * Builds the base Prisma filter used by all dashboard widgets for one user and period.
 *
 * @param {string} userId
...
/**
 * Loads metadata for the categories returned by the grouped expense aggregation.
 *
 * @param {string} userId
 * @param {Array<{ categoryId: string }>} topExpenseGroups
 * @returns {Promise<Map<string, { id: string, name: string, color: string | null }>>}
 */
