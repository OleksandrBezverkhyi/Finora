...
const messages = {
  ...
  uk: {
    ...
    login: {
      eyebrow: "Вхід",
      heroTitle: "Контролюйте доходи й витрати в зручному щоденному процесі.",
      heroText:
        "Слідкуйте за бюджетом, переглядайте щоденні витрати й тримайте всі фінансові рішення в одному впорядкованому просторі.",
      cardOverview: "Огляд",
      ...
      noAccountText: "Створіть акаунт у кілька кроків і почніть тримати особисті фінанси в порядку.",
    },
    register: {
      eyebrow: "Реєстрація",
      heroTitle: "Створіть свій простір перед тим, як увімкнуться бюджети, аналітика й планування.",
      heroText:
        "Створіть акаунт один раз і почніть відстежувати доходи, витрати, цілі заощаджень та щомісячні звички в одному місці.",
      profile: "Профіль",
      ...
      description:
        "Заповніть дані, щоб створити персональний простір для щоденного фінансового обліку. Усі суми в застосунку за замовчуванням ведуться в гривні.",
      yourName: "Ваше ім'я",
...
export function formatMoneyLocalized(value, locale = "en") {
...
export function localizeRecommendationAlert(alert, locale = "en") {
  const normalized = normalizeLocale(locale);
  if (normalized === "en") {
    return alert;
  }
  ...
  if (alert.type === "budget_exceeded") {
    return {
      ...alert,
      title: `${alert.meta.categoryName} - ліміт перевищено`,
      message:
        `Ви витратили ${formatMoneyLocalized(alert.meta.spentAmount, locale)} при ліміті ` +
        `${formatMoneyLocalized(alert.meta.budgetAmount, locale)} у ${formatMonthYearLocalized(new Date(alert.meta.year, alert.meta.month - 1, 1), locale)}.`,
  ...
  if (alert.type === "goal_overdue") {
    return {
      ...
      message:
        ...
        `${formatMoneyLocalized(alert.meta.weeklyAmount, locale)} на тиждень або ` +
        `${formatMoneyLocalized(alert.meta.monthlyAmount, locale)} на місяць.`,
  ...
  if (alert.type === "saving_pace") {
    return {
      ...
      title: `Темп заощаджень для цілі ${alert.meta.goalName}`,
      message:
        `Щоб залишатися в графіку, відкладайте ${formatMoneyLocalized(alert.meta.weeklyAmount, locale)} ` +
        `на тиждень або ${formatMoneyLocalized(alert.meta.monthlyAmount, locale)} на місяць.`,
      actionLabel: "Відкрити цілі",
    };
  }
